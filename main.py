...
app.add_middleware(
    ...
    allow_origins=["http://127.0.0.1:5500", "http://localhost:5500","https://vingsfire-final-widget.vercel.app/","https://vingsfire-chatbot-frontend.onrender.com","https://vingsfire-chatbot.netlify.app/","https://infinitecard.in/"], 
...
# Define a command string that the frontend button will send
...
def generate_and_send_proposal_task(user_details, category, custom_category_name, custom_category_data):
    try:
        ...
        print(f"Starting proposal generation for {user_details['email']}...")
        ...
        print(f"Successfully sent proposal to {user_details['email']}")
    except Exception as e:
        print(f"ERROR in background proposal task: {e}")

# --- NEW: Helper function to go back a stage ---
def go_back_to_stage(previous_stage: str, user_details: Dict[str, Any]) -> ChatResponse:
    # Remove any data associated with the stage we are going back FROM (i.e., the current stage in history)
    ...
    if previous_stage == "get_name":
        ...
        # The welcome message is handled by the frontend, so we just return the buttons
        ...
        # Logic depends on whether App Dev or Other Service
        ...
        else: # Non-App Dev services that have sub-categories
            return ChatResponse(next_stage="get_main_service", bot_message="Which of our main services are you interested in?", user_details=user_details, ui_elements={"type": "buttons", "display_style": "cards", "options": main_services})
            
    elif previous_stage == "get_specific_service":
        # Cannot safely determine what the options were in this specific state, 
        # so we go back to the sub-category stage.
        user_details.pop('category', None)
        user_details.pop('custom_category_name', None) # If it was set during redirection
    ...
    # Default fallback: cannot go back further
...
async def handle_chat(request: ChatRequest):
    ...
    # -----------------------------------------------------------
    # TIER 1A: GLOBAL BACK COMMAND INTERCEPTION
    # -----------------------------------------------------------
    ...
    # -----------------------------------------------------------
    # TIER 1B: GLOBAL FLOW CONTROL INTERCEPTION
    # -----------------------------------------------------------
    if "new proposal" in user_input_lower and user_details.get('name'):
        user_details = {'name': user_details['name'], 'stage_history': []} # Full reset for new flow
        return ChatResponse(
    ...
    if stage in ["general_chat", "final_generation"] and any(phrase in user_input_lower for phrase in ['connect me', 'connect', 'talk to someone', 'talk to a human']):
        return ChatResponse( 
            next_stage='general_chat', 
            bot_message="Of course. Our team is happy to help. You can reach them directly by sending an email to **sales@infinitecard.in**. They will get back to you shortly.", 
            user_details=user_details
        )
    # -----------------------------------------------------------
    
    # Record the current stage for potential future 'Go Back'
    ...
    if stage == "get_name":
        ...
        # App Development skips platform choice
        if user_input == "App Development":
            app_sub_cats = list(app_sub_category_definitions.keys())
            return ChatResponse(
                next_stage="get_sub_category",
                bot_message="Understood. Please select the category that best fits your app idea.",
                user_details=user_details,
                ui_elements={"type": "buttons", "display_style": "cards", "options": app_sub_cats}
            )
        ...
        return ChatResponse(
            next_stage="get_specific_service", 
            bot_message=f"Perfect. Now, which specific type of {user_input} are you looking for?", 
            user_details=user_details, 
        ...
        category_exists = (
            normalized_user_input in services_data.get(main_service, {}).get(sub_cat, {})
        )

        available_keys = services_data.get(main_service, {}).get(sub_cat, {}).keys()
        print(f"DEBUG: Checking Service: '{normalized_user_input}'")
        print(f"DEBUG: Found keys for Sub-Category '{sub_cat}': {available_keys}")
        print(f"DEBUG: Category exists: {category_exists}")
        ...
        if normalized_user_input == "Others" or not category_exists:
            ...
            return ChatResponse(
                ...
                bot_message=f"I don't have a standard estimate for **{normalized_user_input}**. Please briefly describe the application you need (e.g., key features), and I will prepare a custom estimate.", 
                user_details=user_details
            )
        else:
    ...
    # --- FINAL CHAT LOGIC (TIER 2 & 3) ---
    elif stage in ["general_chat", "final_generation", "job_application"]:
        
        # Tier 2: State-Specific Interception (High-Confidence Answers)
        ...
        if any(phrase in user_input_lower for phrase in proposal_keywords):
            return ChatResponse(
                next_stage="general_chat",
                bot_message="Your personalized proposal is being finalized and was sent to your email. Please check your inbox and spam folder. Is there anything else I can assist you with?",
                user_details=user_details
            )
        ...
        if any(phrase in user_input_lower for phrase in exit_keywords):
            return ChatResponse(
                next_stage="ended",
                bot_message="You're welcome! Thank you for your time. Have a great day!",
                user_details=user_details
            )

        # Tier 3: LLM Intent Fulfillment (Catch-all)
